...
const subscriptionSchema = Yup.object().shape({
  ...
  frequency: Yup.string()
    ...
    .oneOf(["monthly", "yearly", "quarterly", "weekly"], "Please select a valid frequency"),
  payment: Yup.string()
    .required("Price is required")
    .test("is-valid-payment", "Please enter a valid price greater than 0", (value) => {
...
function DatePickerInput({ dueDate, setDueDate }: { dueDate: Date | undefined; setDueDate: (date: Date | undefined) => void }) {
...
export function SubscriptionForm({ onSuccess }: SubscriptionFormProps) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm, setSubmitting }) => {
      try {
        ...
        const currencySelect = document.querySelector('[data-currency-select]') as HTMLSelectElement;
        ...
        toast.error(error instanceof Error ? error.message : "An error occurred while adding subscription");
      } finally {
        setSubmitting(false);
      }
    },
  ...
  return (
    <Card className="px-[26px] py-[36px]">
      <CardHeader>
        <CardTitle className="text-[22px] font-bold">Add New Subscription</CardTitle>
      </CardHeader>
        ...
        <form onSubmit={formik.handleSubmit}>
          <div className="grid grid-cols-3 gap-[32px]">
            ...
            <div className="space-y-2">
              <Label htmlFor="subscription-name" className="text-[16px]">Product/Subscription Name:<span className="text-red-500">*</span></Label>
              <Input
            ...
            <div className="space-y-2">
              <Label htmlFor="functions" className="text-[16px]">Functions:<span className="text-red-500">*</span></Label>
              <Select value={formik.values.functions} onValueChange={(value) => value && formik.setFieldValue('functions', value)}>
                <SelectTrigger className={`!h-[44px] !min-h-[44px] px-3 py-0 flex items-center justify-between w-full rounded-[8.831px] border bg-background shadow-sm text-foreground dark:bg-[#222] dark:border-[#474747] dark:shadow-[0_1.656px_4.415px_-1.104px_rgba(10,9,11,0.07)] dark:text-white ${formik.errors.functions ? 'border-red-500' : ''}`}>
              ...
              {formik.errors.functions && formik.touched.functions && <p className="text-red-500 text-sm mt-1">{formik.errors.functions}</p>}
            </div>
            <div className="space-y-2">
              <Label htmlFor="frequency" className="text-[16px]">Subscription Frequency:<span className="text-red-500">*</span></Label>
              <Select value={formik.values.frequency} onValueChange={(value) => value && formik.setFieldValue('frequency', value)}>
                <SelectTrigger className={`!h-[44px] !min-h-[44px] px-3 py-0 flex items-center justify-between w-full rounded-[8.831px] border bg-background shadow-sm text-foreground dark:bg-[#222] dark:border-[#474747] dark:shadow-[0_1.656px_4.415px_-1.104px_rgba(10,9,11,0.07)] dark:text-white ${formik.errors.frequency ? 'border-red-500' : ''}`}>
              ...
              {formik.errors.frequency && formik.touched.frequency && <p className="text-red-500 text-sm mt-1">{formik.errors.frequency}</p>}
            </div>

            {/* Row 2 */}
            <div className="space-y-2">
              <Label htmlFor="subscription-email" className="text-[16px]">Subscription Email:<span className="text-red-500">*</span></Label>
              <Input
            ...
            <div className="space-y-2">
              <Label htmlFor="price" className="text-[16px]">Price<span className="text-red-500">*</span></Label>
              <div className="flex">
                <Select>
                  <SelectTrigger className="!h-[44px] !min-h-[44px] px-3 py-0 flex items-center justify-between w-[120px] rounded-r-none border-r-0 bg-background border shadow-sm text-foreground dark:bg-[#222] dark:border-[#474747] dark:shadow-[0_1.656px_4.415px_-1.104px_rgba(10,9,11,0.07)] dark:text-white" data-currency-select>
              ...
              {formik.errors.payment && formik.touched.payment && <p className="text-red-500 text-sm mt-1">{formik.errors.payment}</p>}
            </div>

            <div className="space-y-2">
              <Label htmlFor="due-dates" className="text-[16px]">Due Dates</Label>
              ...
              <button
                ...
                {(formik.isSubmitting || createSubscription.isPending) && (
                  <svg
                    ...
                    viewBox="0 0 24 24"
                  >
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 12v8m0 0l6-6m-6 6H6" />
                  </svg>
                )}

                {/* Orange circle (same height as button) */}
                <span className={`absolute left-0 top-0 h-full w-[56px] bg-[#FF5A24] rounded-full transition-all duration-500 ease-in-out
                ${(formik.isSubmitting || createSubscription.isPending) ? 'w-[56px]' : 'group-hover:w-full'} group-hover:rounded-full`}></span>

                {/* Text */}
                <span className="relative z-10 text-foreground text-lg font-medium dark:text-white">
                  {(formik.isSubmitting || createSubscription.isPending) ? 'Adding...' : 'Add Subscription'}
                </span>
